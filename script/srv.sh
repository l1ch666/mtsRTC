...
REPO_URL="${OLCRTC_REPO_URL:-}"
WORK_DIR="/tmp/olcrtc-deploy-$PODMAN_ID"
BRANCH="${OLCRTC_BRANCH:-local}"
NO_CACHE=0
...
echo "  4) wbstream"
echo "  5) mtslink"
read -p "Enter choice [1-5, default: 1]: " CARRIER_CHOICE
...
elif [ "$CARRIER" = "mtslink" ]; then
    echo "MTS Link room URL example: https://my.mts-link.ru/j/167846474/19645959806"
    read -p "Enter MTS Link room URL: " ROOM_ID
    if [ -z "$ROOM_ID" ]; then
        echo "[X] MTS Link room URL cannot be empty"
        exit 1
    fi
    case "$ROOM_ID" in
        http://*|https://*) ;;
        *) ROOM_ID="https://$ROOM_ID" ;;
    esac
    read -p "Optional MTS_COOKIE for non-public rooms (leave empty for guest): " MTS_COOKIE_INPUT
    MTS_COOKIE=${MTS_COOKIE_INPUT:-}
else
    read -p "Enter Room ID: " ROOM_ID
    if [ -z "$ROOM_ID" ]; then
...
if [ "$CARRIER" = "mtslink" ]; then
    ...
    VIDEO_CODEC="qrcode"
    VIDEO_QR_SIZE=0
    VIDEO_QR_RECOVERY="low"
fi

if [ "$TRANSPORT" = "videochannel" ] && [ "$CARRIER" != "mtslink" ]; then
    echo ""
...
if [ "$CARRIER" = "mtslink" ] && [ -n "${MTS_COOKIE:-}" ]; then
    echo "MTS_COOKIE:     provided"
fi
echo ""
TRANSPORT_PAYLOAD=""
if [ "$TRANSPORT" = "vp8channel" ]; then
...
ROOM_ID_URI=$(urlencode "$ROOM_ID")
SUB_CONFIGNAME_URI=$(urlencode "$sub_configname")
OLC_URI="olcrtc://$CARRIER?${TRANSPORT}${TRANSPORT_PAYLOAD}@${ROOM_ID_URI}#${KEY}\$${SUB_CONFIGNAME_URI}"
echo "uri: $OLC_URI"
echo ""
