...
func bindingTokens(cfg transport.Config) []uint32 {
	tokens := make([]uint32, 0, 6)
	tokens = appendBindingToken(tokens, bindingID(cfg))
	tokens = appendRoomBindingTokens(tokens, cfg.Carrier, cfg.RoomURL)
...
func appendRoomBindingTokens(tokens []uint32, carrier, roomURL string) []uint32 {
	roomURL = strings.TrimSpace(roomURL)
	tokens = appendBindingToken(tokens, roomURL)
	if strings.EqualFold(strings.TrimSpace(carrier), "telemost") {
		tokens = appendBindingToken(tokens, canonicalTelemostRoomURL(roomURL))
		tokens = appendBindingToken(tokens, bareTelemostRoomID(roomURL))
	}
	return tokens
}

func canonicalTelemostRoomURL(roomURL string) string {
	roomID := bareTelemostRoomID(roomURL)
	if roomID == "" || strings.HasPrefix(roomID, "http://") || strings.HasPrefix(roomID, "https://") {
		return ""
	}
	return "https://telemost.yandex.ru/j/" + roomID
}

func bareTelemostRoomID(roomURL string) string {
	roomURL = strings.TrimRight(strings.TrimSpace(roomURL), "/")
	const prefix = "https://telemost.yandex.ru/j/"
	if strings.HasPrefix(roomURL, prefix) {
		return strings.TrimRight(strings.TrimPrefix(roomURL, prefix), "/")
	}
	return roomURL
}
