...
func TestBindingTokensPreferClientIDAndKeepRoomURLFallback(t *testing.T) {
	...
	if !hasToken(tokens, bindingToken("79061977050409")) {
		t.Fatalf("tokens = %#v, want bare Telemost room id fallback", tokens)
	}
...
func TestBindingTokensAcceptTelemostBareAndFullRoomForms(t *testing.T) {
	full := "https://telemost.yandex.ru/j/79061977050409"
	bare := "79061977050409"

	fromBare := bindingTokens(transport.Config{Carrier: "telemost", RoomURL: bare})
	if !hasToken(fromBare, bindingToken(full)) {
		t.Fatalf("tokens from bare room = %#v, want full URL fallback", fromBare)
	}
	if !hasToken(fromBare, bindingToken(bare)) {
		t.Fatalf("tokens from bare room = %#v, want bare room token", fromBare)
	}

	fromFull := bindingTokens(transport.Config{Carrier: "telemost", RoomURL: full})
	if !hasToken(fromFull, bindingToken(bare)) {
		t.Fatalf("tokens from full URL = %#v, want bare room fallback", fromFull)
	}
	if !hasToken(fromFull, bindingToken(full)) {
		t.Fatalf("tokens from full URL = %#v, want full URL token", fromFull)
	}
}

func hasToken(tokens []uint32, want uint32) bool {
